...
namespace detail {
    ...
    public:
...
#ifdef AZMQ_DETAIL_USE_IO_SERVICE
            void do_attach(boost::asio::io_service & ios,
#else
            void do_attach(boost::asio::io_context & ios,
#endif			 
                         context_type & ctx,
                         void *sock,
                         bool optimize_single_threaded,
                         boost::system::error_code & ec) {
                BOOST_ASSERT_MSG(!socket_, "socket already open");
                BOOST_ASSERT_MSG(sock, "socket is null");
                socket_.reset(sock);
                socket_attached_ = true;
                if (ec) return;
        ...
        boost::system::error_code do_attach(implementation_type & impl,
                                          void *sock,
                                          bool optimize_single_threaded,
                                          boost::system::error_code & ec) {
            BOOST_ASSERT_MSG(impl, "impl");
#ifdef AZMQ_DETAIL_USE_IO_SERVICE
            impl->do_attach(get_io_service(), ctx_, sock, optimize_single_threaded, ec);
#else
            impl->do_attach(get_io_context(), ctx_, sock, optimize_single_threaded, ec);
#endif
            if (ec)
                impl.reset();
            return ec;
